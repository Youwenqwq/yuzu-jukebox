...
- **Artist profiles**: `GET /api/v1/artists/{name}` aggregates `play_history` by exact artist name (play_count = "monthly listeners" stand-in, top 10 tracks with proxy covers), then best-effort enriches avatar/bio via the optional `provider.ArtistDetailer` interface — ncm (`/search` type=100 first hit → `/artist/detail` briefDesc; `/artist/desc` introduction is available upstream but not wired), qq (`/singer/{mid}/desc`, anonymous, zero sidecar changes), bili (`/search/up` face/sign snapshot; upgrades to `/space/acc/info` with a credential, SESSDATA ≥3 items). Enrichment failure only omits fields — never fabricates. The name is the join key; multi-artist tracks match the joined string (`"A/B"`). Providers are tried in ID order (bili first) — name-keyed matching is inherently fuzzy across platforms.
...
- **Radio sources**: `capabilities.radio_sources` in `/api/v1/providers` comes from the optional `provider.SourceCatalog` interface; `finite=false` sources reject `shuffle`/`once`. The generic `playlist:<id>` source is room-level, always available, and deliberately absent from the catalog. Anonymous finite sources added: bili `ranking:<key|rid>` (music/kichiku/all via `/region-ranking`, no WBI), ncm `newsong` (`/personalized/newsong`). Dynamic catalogs (qq toplists) enumerate via `GET /api/v1/providers/{id}/radio-catalog` (`provider.RadioSourceCatalogLister`; qq maps `/top/get_category` → `top:<id>` with covers; 404/501/502 per capability contract).
