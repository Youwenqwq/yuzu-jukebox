...
- **Artist profiles**: `GET /api/v1/artists/{name}` aggregates `play_history` by exact artist name (play_count = "monthly listeners" stand-in, top 10 tracks with proxy covers), then best-effort enriches avatar/bio via the optional `provider.ArtistDetailer` interface (ncm implements: `type=100` search first hit → `/artist/detail`). Enrichment failure only omits fields — never fabricates. The name is the join key; multi-artist tracks match the joined string (`"A/B"`).
- **Recommendations feed**: `GET /api/v1/recommendations` aggregates shelves from all `provider.RecommendationProvider` implementers (ncm: `/toplist` top 3 → `/toplist/detail`, 10 tracks/shelf). Per-provider failures are logged and skipped; no source → `{"shelves":[]}` 200. `provider.Registry.All()` is sorted by ID — callers depend on deterministic provider order (enrichment priority, feed aggregation, listProviders response).
