...
- **NCM credential probe reads a nested envelope**: `/login/status` (upstream NeteaseCloudMusicApi and the Enhanced fork alike) nests the business code inside `data.code` — the top level has **no** `code` field, so the generic `getWithClient` top-level `code==200` check must not be used there. `checkLogin` fetches the raw body via `doGet` and judges by `data.profile`/`data.account` presence: both null → not logged in (Enhanced returns `code:200` + nulls for a garbage cookie; the original returns `code:301` unwrapped — both land in the same "not logged in" branch).
...
- **NCM chart catalog**: `RadioSourceCatalog()` maps anonymous `/toplist` results to finite `toplist:<id>` entries with raw `coverImgUrl`; the HTTP aggregation layer prefixes them as `ncm:toplist:<id>` and mints entity-cover proxy URLs. Materialization reuses `/playlist/detail` + `/playlist/track/all` because NCM toplists are upstream playlists.
...
- **QQ credential validation is asymmetric**: `SetCredential` refreshes-and-stores when the credential carries `refresh_token`/`refresh_key` (QR logins always do), and falls back to structural validation (musicid+musickey present) for minimal admin-pasted cookies. `CredentialStatus` first does a local expiry check via `musickey_create_time`+`key_expires_in`, then probes `/login/check_expired`. The sidecar's `check_expired` IS a real server probe (WEB platform: `fcg_get_profile_homepage.fcg` with g_tk; else `GetLoginUserInfo`) — garbage musickey yields nonzero code → expired. Its bool result rides the sidecar's unified envelope for all bool routes: `true`(已过期) → `{code:0, data:null}`, `false`(健康) → `{code:-1, msg:"操作失败"}` — yuzu's `checkExpired` parses exactly this (and tolerates a future `{code:0, data:bool}` shape). `get_vip_info` stays a weak signal (accepts garbage, returns zeros).
