...
```jsonc
{
  ...
  "media": {"max_upload_bytes": 1073741824}, // 单次上传上限，默认 1 GiB
  "cache": {"max_object_bytes": 536870912},  // 单个远端缓存对象，默认 512 MiB
  // ...
}
```
...
- `cache.max_object_bytes`：限制单个远端对象的下载与临时文件大小；设为 `0`
  可显式关闭该限制。它独立于控制缓存总容量的 `cache_max_bytes`。
- 缓存下载不再设整轨 10 分钟总时限；等待上游响应头或单次读取连续 30 秒无响应时终止。
  播放器两次读取之间的暂停不计入上游读取超时，断开探测连接后仍可后台补完缓存。
  读取/落盘失败及已知长度不匹配不会作为完整缓存发布。
- 冷缓存的 `Range` 请求仍需等待完整文件下载；这是当前读取模型的边界，不提供渐进 Range。
  `cache_max_bytes` 应足以容纳需要缓存的最大文件，否则刚完成的文件可能立即被 LRU 回收。
