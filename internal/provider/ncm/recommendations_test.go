...
// TestRecommendations 榜单 feed：/toplist 前 3 榜 → /toplist/detail，
// 每榜曲目封顶 10；单个榜单详情失败时跳过该 shelf 保留其余。
func TestRecommendations(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/toplist/detail":
			...
			case "11":
				...
				sb.WriteString(`{"code":200,"list":{"id":11,"name":"飙升榜","tracks":[`)
				for i := 1; i <= 12; i++ {
					if i > 1 {
						sb.WriteString(",")
					}
			...
			case "12":
				_, _ = w.Write([]byte(`{"code":200,"list":{"id":12,"name":"新歌榜","tracks":[{"id":101,"name":"New Song","duration":2000,"al":{"name":"New Album","picUrl":"https://cover/101"},"ar":[{"name":"New Artist"}]}]}}`))
			case "13":
				// 详情失败 → 该 shelf 被跳过
	...
	if len(shelves) != 2 {
		t.Fatalf("shelves = %#v, want 2 (13 详情失败被跳过)", shelves)
	}
	...
	if strings.Join(detailCalls, ",") != "11,12,13" {
		t.Fatalf("detail calls = %v, want 11,12,13", detailCalls)
	}
}
...
// TestRecommendationsAllDetailsFail 全部详情失败 → 整体报错。
func TestRecommendationsAllDetailsFail(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/toplist":
	...
	if _, err := p.Recommendations(context.Background()); err == nil {
		t.Fatal("Recommendations = nil error, want error when all details fail")
	}
}
