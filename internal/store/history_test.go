...
func seedPlayHistory(t *testing.T, st *Store) {
	...
	ctx := context.Background()
	rows := []struct {
		roomID, ref, title, requester, reason string
		startedAt                             int64
	}{
		{"room-1", "ncm:a", "A old", "alice", "finished", 100},
		{"room-2", "ncm:a", "A middle", "bob", "finished", 200},
		{"room-2", "ncm:b", "B old", "alice", "finished", 300},
		{"room-1", "ncm:c", "C", "alice", "skipped", 350},
		{"room-2", "ncm:a", "A new", "alice", "finished", 500},
		{"room-1", "ncm:b", "B new", "bob", "error", 600},
	}
	for _, row := range rows {
		if err := st.AddPlayHistory(ctx, row.roomID, row.ref, row.title, row.requester, row.startedAt, row.startedAt+1, row.reason); err != nil {
			t.Fatal(err)
		}
	}
}
